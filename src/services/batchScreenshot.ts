...
export const batchScreenshotWithRetry = async (
  ...
  // BUG: loose equality check — should use !== (strict)
  ...
  // BUG: missing await — updateDoc returns a Promise
...
const screenshotWithRetry = async (
  ...
  for (let attempt = 1; attempt <= policy.maxRetries; attempt++) {
    ...
    // BUG: catch(error: any) — should use unknown
    } catch (error: any) {
 ...
 * BUG: sets cancelled flag but batchScreenshotWithRetry never checks it
 */
...
export const getBatchScreenshotHistory = async (projectId: string, limit: number = 10) => {
  ...
  // BUG: slice before sort — should sort first, then slice
